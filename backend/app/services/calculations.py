...
def calculate_all_tariffs(file_obj, db: Session) -> dict:
    ...
    if "Data i godzina" in df.columns and "Wartosc[kWh/kvar]" in df.columns:
        ...
        raise HTTPException(
            status_code=422,
            detail="Nierozpoznany format pliku. Brak wymaganych kolumn.",
        )
    ...
    text_dates = pd.to_datetime(
        df.loc[~is_numeric, "Data"],
        format="mixed",
        dayfirst=is_day_first,
        errors="coerce",
    )
    numeric_dates = pd.to_datetime(
        pd.to_numeric(df.loc[is_numeric, "Data"]), unit="D", origin="1899-12-30"
    )
    ...
    df_15min["Dokładny Czas"] = base_timestamps + pd.to_timedelta(
    ...
    db_prices = (
        ...
        .filter(DynamicPrice.date >= min_date, DynamicPrice.date <= max_date)
        .all()
    )

    dynamic_prices_dict = {
        (str(p.date), str(p.hour)): float(p.price_per_kwh) for p in db_prices
    }
    ...
    data_15min = (
        df_15min.groupby("time_15m")[columns_to_aggregate].mean().round(2).reset_index()
    )
    ...
    daily_hourly_sum = (
        df_15min.groupby(["date", "hour"])[columns_to_aggregate].sum().reset_index()
    )

    hourly_data = (
        daily_hourly_sum.groupby("hour")[columns_to_aggregate]
        .mean()
        .round(2)
        .reset_index()
    )
    ...
    daily_data = (
        df_15min.groupby("date")[columns_to_aggregate].sum().round(2).reset_index()
    )
