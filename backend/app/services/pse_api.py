...
def ensure_dynamic_prices(start_date: date, end_date: date, db: Session):
    existing_days = db.query(DynamicPrice.date)\
            ...
            if data.get("value"):
                ...
                for item in data["value"]:
                    ...
                    if "-" in hour_str:
                        ...
                        exists = db.query(DynamicPrice).filter_by(date=item_date, hour=time_label).first()
                        if not exists:
                            new_prices.append(DynamicPrice(
                                date=item_date, hour=time_label, price_per_kwh=round(price_per_kwh, 4)
                            ))
                    else:
